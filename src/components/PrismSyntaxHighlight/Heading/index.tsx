...
const heading =
  ...
  ({ children }: HeadingProps) => {
    ...
    if (level === 4) {
      ...
      return (
        <Title level={4} id={`#${children}`} className={cx(styles.heading, styles.levelStyle, `h${level}`)}>
          <span>
            <a id={`${children}`} className={styles.link} onClick={(e) => handleClick(e, children)}>
    ...
    return (
      <Title level={level} id={children as string} className={cx(styles.heading, styles.levelStyle, `h${level}`)}>
        {children}
      </Title>
    )
  }
