...
// 生成 OKLCH 色板
export const generateColorPalette = (baseColorHex: string, options: ColorPaletteOptions = {}): Color[] => {
  ...
  for (let i = lighter.steps; i > 0; i--) {
    ...
    const chromaValue =
      baseColorOKLCH[1] -
      (((1 - lighter.saturationAdjustment) * lighter.saturationScale * baseColorOKLCH[1]) / lighter.steps) * i
  ...
  for (let i = 1; i <= darker.steps; i++) {
    ...
    const chromaValue =
      baseColorOKLCH[1] -
      (((1 - darker.saturationAdjustment) * darker.saturationScale * baseColorOKLCH[1]) / darker.steps) * i
...
export const generateNeutralPalette = (baseColorHex: string, options: NeutralPaletteOptions = {}): Color[] => {
  const baseColor = chroma(baseColorHex)
  const baseColorOKLCH = baseColor.oklch()
