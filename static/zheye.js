...
function modifyAppConfig() {
  ...
  try {
    if (!!$.response.body) {
      ...
      obj["config"]["homepage_feed_tab"]["tab_infos"] = obj["config"]["homepage_feed_tab"]["tab_infos"].filter((e) => {
  ...
  try {
    if (!!$.response.body) {
      ...
      if (obj.more_sections) {
        let sectionsToRemove = $.data.read("zheye_remove_feed_tabs", "").split(";")
        obj.selected_sections = obj.selected_sections.filter((item) => !sectionsToRemove.includes(item["section_name"]))
    ...
    if ($.request.method === "GET") {
      ...
      if (!!$.request.body) {
        ...
        if (keywordExists === false) {
          ...
          let body = JSON.stringify({ error: { message: "关键词已存在", code: 100002 } })
          if ($.env.isQuanX) {
...
function processUserInfo() {
  ...
  try {
    ...
    if (obj && obj["id"] && obj.hasOwnProperty("vip_info") && obj["vip_info"].hasOwnProperty("is_vip")) {
      const userInfo = { id: obj["id"], is_vip: obj["vip_info"]["is_vip"] ? obj["vip_info"]["is_vip"] !== undefined : false }
      ...
      if ($.data.read("zhihu_settings_fake_vip") !== false && obj["vip_info"]["is_vip"] === false) {
        ...
        obj["vip_info"]["entrance_new"] = {
          left_button: { title: "精选会员内容", description: "为您严选好内容", jump_url: "zhihu://market/home" },
          right_button: { title: "开通盐选会员", description: "畅享 10w+ 场优质内容等特权", jump_url: "zhihu://vip/purchase" },
        }
...
function manageBlackUser() {
  ...
  if ($.request.method === "GET") {
    ...
    try {
      ...
      if (obj.hasOwnProperty("name") && obj.hasOwnProperty("id")) {
        ...
        if (obj["id"]) {
          ...
          $.data.write(blockedUsersKey, customBlockedUsers, userInfo.id)
          $.logger.debug(`${obj["name"]}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(customBlockedUsers)}`)
    ...
    try {
      let obj = JSON.parse($.response.body)
      if (obj.success) {
        let user_id = $.request.url.match(/^https?:\/\/api\.zhihu\.com\/settings\/blocked_users\/([0-9a-zA-Z]*)/)[1]
        if (user_id) {
          ...
          for (let username in customBlockedUsers) {
            if (customBlockedUsers[username] === user_id) {
              ...
              $.data.write(blockedUsersKey, customBlockedUsers, userInfo.id)
              $.logger.debug(`${username}移出脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(customBlockedUsers)}`)
...
function _setRecommendTag(item, text, color = "GBK02A") {
  ...
  item["common_card"]["footline"]["elements"].unshift({ tag: { text: text, color: color, type: "MASK_ROUNDED_RECTANGLE" } })
}
async function _checkPaidContentByCloud(filteredUrls, newData) {
  ...
  if (!zheyeServer) {
    ...
    $.logger.debug(`服务端地址\n${url}`)
    await $.http
      .post({ url: url, headers: { "Content-Type": "application/json" }, body: filteredUrls })
      .then((resp) => {
        $.logger.debug(`云端探测结果<${typeof resp.body}>\n${JSON.stringify(resp.body)}`)
      ...
      if (!url || url === "" || !url.startsWith("https://www.zhihu.com/appview/v2/answer")) {
        ...
        $.http
          ...
          .then((resp) => {
            ...
            if ((body.indexOf("查看完整内容") >= 0 || body.indexOf("查看全部章节") >= 0) && body.indexOf("paid") >= 0) {
              resolve("付费内容")
            } else if (body.indexOf("ad-link-card") >= 0 || body.indexOf("xg.zhihu.com") >= 0 || body.indexOf("营销平台") >= 0) {
              resolve("营销推广")
...
async function removeRecommend() {
  ...
  try {
    ...
    const settings_check_paid_content = $.data.read("zhihu_settings_check_paid_content", false)
    const settings_request_content = $.data.read("zhihu_settings_request_content", "local")
    const user_info = getUserInfo()
    ...
    const dataFilter = (element) => {
        ...
        for (let i = 0; i < keywords.length; i++) {
          if (elementStr.search(keywords[i]) >= 0) {
            if ($.isDebug) {
              ...
              $.logger.debug(`匹配关键字：\n${keywords[i]}\n标题：\n${elementTitle}\n内容：\n${elementContent}`)
              $.notification.debug(scriptName, `关键字：${keywords[i]}`, `${elementTitle}\n${elementContent}`, actionUrl)
            }
            matchKeyword = true
            break
          }
      ...
      try {
        isBlockedUser =
          ...
          settings_blocked_users &&
          customBlockedUsers &&
          element["common_card"]["feed_content"]["source_line"]["elements"][1]["text"]["panel_text"] in customBlockedUsers
      } catch {
        isBlockedUser = false
      }
    ...
    if (settings_check_paid_content === true) {
      ...
      for (let i = 0; i < newData.length; i++) {
        if (newData[i]) {
          try {
            let url = newData[i]["common_card"]["feed_content"]["title"]["action"]["intent_url"]
            url = url.replace(/^https:\/\/zhihu\.com\/question\/\d+\/answer\//, "https://www.zhihu.com/appview/v2/answer/")
      ...
      } else if (filteredUrls.length > 0 && settings_request_content === "local") {
            ...
            if (comment.child_comments) {
              ...
              comment.child_comments.forEach((childComment) => {
                ...
                const childCommentReplyUserName =
                  typeof childComment["reply_to_author"] != "undefined" ? childComment["reply_to_author"].name : ""
  ...
  try {
    if (!!$.response.body) {
      ...
      obj.data.forEach((element) => {
        ...
        const isBlackUser = typeof customBlockedUsers[authorName] != "undefined"
    ...
    if (!!$.response.body) {
      ...
      obj.data.forEach((element) => {
        ...
        const isAd =
          ...
          (element["biz_type_list"] && element["biz_type_list"][0] !== "answer") ||
          element["type"] === "ad"
        if (isAd === true) {
          return
        }
  ...
  if ($.isResponse) {
    switch (true) {
      case /^https:\/\/api\.zhihu\.com\/people\/self$/.test($.request.url):
        ...
        break
      case $.data.read("zhihu_settings_app_conf", false) === true && /^https?:\/\/appcloud2\.zhihu\.com\/v\d+\/config/.test($.request.url):
      ...
      case $.data.read("zhihu_settings_app_conf", false) === true &&
        ...
        break
      case $.data.read("zhihu_settings_app_conf", false) === true &&
        /^https?:\/\/api\.zhihu\.com\/feed\/render\/tab\/config/.test($.request.url):
      ...
      case $.data.read("zhihu_settings_answer_tip", true) === true &&
        ...
        break
      case $.data.read("zhihu_settings_blocked_users", false) !== false && /^https?:\/\/api\.zhihu\.com\/next\?/.test($.request.url):
...
function MagicJS(scriptName = "MagicJS", logLevel = "INFO") {
  ...
  return new (class {
    constructor(scriptName, logLevel) {
      ...
      this.notification =
        typeof MagicNotification === "function" ? MagicNotification(this.scriptName, this.env, this.logger, this.http) : undefined
...
function MagicNotification(scriptName, env, logger, http) {
  ...
  function bark(title = scriptName, subTitle = "", body = "", opts = "") {
    ...
    let options = {
      ...
      body: { title: title, body: subTitle ? `${subTitle}\n${body}` : body, device_key: _barkKey },
    }
...
function MagicHttp(env, logger) {
  ...
  const mergeConfig = (method, configOrUrl) => {
    let config = typeof configOrUrl === "object" ? { headers: {}, ...configOrUrl } : { url: configOrUrl, headers: {} }
    if (!config.method) {
  ...
  const interceptors = { request: new InterceptorManager(), response: new InterceptorManager(false) }
    ...
    function requestTimeout(config) {
      try {
        const timer = new Promise((_, reject) => {
          setTimeout(() => {
            let err = { message: `timeout of ${config["timeout"]}ms exceeded.`, config: config }
            reject(err)
          }, config["timeout"])
        })
